...
namespace MyBookShop.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public class InputModel
        {
            ...
            /// </summary>
            [DataType(DataType.Password)]
            [Display(Name = "Подтвердите пороль")]
        ...
        public async Task OnGetAsync(string returnUrl = null)
        {
            if (!await _roleManager.RoleExistsAsync(WC.AdminRole))
            {
                ...
                await _roleManager.CreateAsync(new IdentityRole(WC.CostumerRole));
            }
            ReturnUrl = returnUrl;
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    {
                        await _userManager.AddToRoleAsync(user, WC.CostumerRole);
                    }
                    ...
                    await _emailSender.SendEmailAsync(Input.Email, "Подтвердите свой адрес электронной почты",
                        $"Для подтверждения перейдите по сслыке <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>по сслыке</a>.");
